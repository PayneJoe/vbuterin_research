...
class Point():
    ...
    def to_extended(self):
        ox = np.zeros(self.x.shape + (4,), dtype=np.uint32)
        ox[...,0] = self.x
        oy = np.zeros(self.y.shape + (4,), dtype=np.uint32)
        oy[...,0] = self.y
...
G = Point(
    np.array(1268011823, dtype=np.uint32),
    np.array(2, dtype=np.uint32)
)

print("Zero point: {}".format(Z))
print("Generator point: {}".format(G))

print(G.shape)
print(G.to_extended())
print(G.to_extended().x.shape[-1])